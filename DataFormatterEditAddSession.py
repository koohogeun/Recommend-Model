...
def process_seqs(iseqs, idates, ses_id, _overall):# make sub graph#for ovarall
    out_seqs = []
    out_dates = []
    labs = []
    ids = []
    overall = []#for ovarall
    max_length = 0
    for seq, date, ses, ova in zip(iseqs, idates, ses_id, _overall):#for ovarall
        ...
        for i in range(1, len(seq)):
            ...
            labs += [tar]
            out_seqs += [seq[:-i]]
            overall += [ova[:-i]]#for ovarall
            out_dates += [date]
            ids += [ses]
    return out_seqs, out_dates, labs, ids, max_length, overall#for ovarall
...
def get_k(filepath, k):
    ...
    with open(dataset, "r") as f:
        ...
        for data in reader:
            ...
            #item = data['item_id'], int(data['timeframe'])
            item = data['item_id'], int(data['timeframe']), int(float(data['overall']))#for overall
            curdate = ''
            curdate = data['eventdate']
        ...
        for i in list(sess_clicks):
            sorted_clicks = sorted(sess_clicks[i], key=operator.itemgetter(1))
            #sess_clicks[i] = [c[0] for c in sorted_clicks]
            sess_clicks[i] = [(c[0],c[2]) for c in sorted_clicks]#for overall
        sess_date[curid] = date
    ...
    for s in list(sess_clicks):
        curseq = sess_clicks[s]
        #filseq = list(filter(lambda i: iid_counts[i] >= 5, curseq))
        filseq = list(filter(lambda i: iid_counts[i[0]] >= 5, curseq))#for overall
        if len(filseq) < 2:
            del sess_clicks[s]
            del sess_date[s]
        else:
...
def make_tr_ts_data_set(tr, ts, sess_clicks):
    ...
    train_data_obj = (tra_ids, tra_dates, tra_seqs)
    test_data_odj = (tes_ids, tes_dates, tes_seqs)
    tr_seqs, tr_dates, tr_labs, tr_ids, tr_max_len, tr_ova = process_seqs(tra_seqs, tra_dates, tra_ids, tra_ova)#for ovarall
    te_seqs, te_dates, te_labs, te_ids, ts_max_len = process_seqs_no_sub_graph(tes_seqs, tes_dates, tes_ids)
    tra = (tr_seqs, tr_labs, tr_max_len, tr_ova)#for ovarall
    tes = (te_seqs, te_labs, ts_max_len)
