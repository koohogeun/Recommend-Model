...
#         print( 'cknn.predict_next: ', process.memory_info().rss, ' memory used')
    ...
    def predict(self, ses_id, it_id, its_id):
        ...
        nexts = ret.copy()
        vals = ret.to_numpy()
        label = ret.index.values
        vals_u = np.unique(vals)
        vals_S = np.sort(vals_u)[::-1][:20 if vals_u.shape[0] > 21 else vals_u.shape[0]]
        idx_arr = np.empty((1,))
        for val_idx in vals_S:
            if val_idx == 0:
                continue
            val_idx_list = np.where(vals == val_idx)[0]
            idx_arr = np.r_[idx_arr, val_idx_list]
            idx_arr = np.unique(idx_arr)
        idx_arr[np.isnan(idx_arr)] = 0
        idx_arr = idx_arr[1:].astype('int32')
        rec_items = label[idx_arr]
        return rec_items[:20 if rec_items.shape[0] > 21 else rec_items.shape[0]], scr
    
    def predict_tr(self, ts_data):
        scores = np.zeros(shape=20)
        targets = np.zeros(shape=1)
        its_id = np.unique(self.data.loc[:,'item_id'].values)
        scr_r = []
        for ins in ts_data.inputs:
            ins = np.delete(ins, np.where(ins == 0))
            if ins[-1] in its_id:
                items, scr = self.predict(self.data.loc[ins[-1] - 1,'session_id'].astype('int32'), ins[-1], its_id)
                scr_r.append(scr)
            else:
                items = np.zeros(shape=20)
            targets = np.r_[targets, items[0]]
            if 20 - items.shape[0] > 0:
                items = np.pad(items, (0,20 - items.shape[0]), 'constant', constant_values=0)
            else:
                items = items[:20]
            scores = np.r_[scores, items]
